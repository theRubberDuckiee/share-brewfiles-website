...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!response.ok) {
      throw new Error(
        "We failed to fetch your user information from the GitHub API."
      );
    }
    ...
    if (!userInfo || !userInfo.username) {
      throw new Error(
        "The user information we got from Github API is invalid."
      );
    }
    ...
    const q = query(
      collectionRef,
      where("userInfo.username", "==", userInfo.username)
    );
    ...
    if (!existingDocs.empty) {
      ...
      return new Response(
        JSON.stringify({ success: true, id: docRef.id, created: true }),
        {
          status: 200,
          headers: {
            "Content-Type": "application/json",
